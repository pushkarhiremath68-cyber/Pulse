(function() {
  ...
  window.selectCatalogCategory = function(catId, btnElement) {
    document.querySelectorAll('.catalog-chip-bar .chip-btn').forEach(btn => btn.classList.remove('active'));
    if (btnElement) btnElement.classList.add('active');

    if (catId === 'all') {
      document.querySelectorAll('.category-horizontal-row').forEach(row => row.classList.remove('hidden'));
    } else {
      const targetRow = document.getElementById(`category-row-${catId}`);
      if (targetRow) {
        document.querySelectorAll('.category-horizontal-row').forEach(row => {
          if (row.id === `category-row-${catId}`) {
        ...
        window.openGenreGridView(catId);
      }
    }
  };

  window.openGenreGridView = function(catId) {
    if (!catId) return;
    const cat = window.catalogService ? window.catalogService.getCategory(catId) : null;
    const tracks = window.catalogService ? window.catalogService.getCategoryTracks(catId) : [];

    const iconWrap = document.getElementById('genre-view-icon-wrap');
    const iconEl = document.getElementById('genre-view-icon');
    const tagEl = document.getElementById('genre-view-tag');
    const titleEl = document.getElementById('genre-view-title');
    const subEl = document.getElementById('genre-view-subtitle');
    const countEl = document.getElementById('genre-view-count');
    const tracksContainer = document.getElementById('genre-grid-tracks-container');

    if (cat) {
      if (titleEl) titleEl.textContent = cat.title || 'Genre Category';
      if (subEl) subEl.textContent = cat.subtitle || 'Explore top tracks in this category';
      if (tagEl) tagEl.textContent = cat.id.toUpperCase();
      if (iconEl && cat.icon) iconEl.className = `fa-solid ${cat.icon}`;
      if (iconWrap && cat.color) iconWrap.style.color = cat.color;
    }
    if (countEl) countEl.textContent = `${tracks.length} songs`;

    if (tracksContainer) {
      tracksContainer.innerHTML = tracks.map(createMusicCardHTML).join('');
    }

    currentGenreTracks = tracks;
    currentActiveGenreCategory = catId;
    switchView('view-genre-grid');
  };

  window.playGenreTracks = function(shuffle = false) {
    if (!currentGenreTracks || currentGenreTracks.length === 0) return;
    let list = [...currentGenreTracks];
    if (shuffle) {
      list = list.sort(() => Math.random() - 0.5);
    }
    state.queue = list;
