...
(function() {
  ...
  // 5. Lyrics Sheet Drawer & Gemini AI Lyrics Generator
  window.toggleLyricsDrawer = function() {
    const drawer = document.getElementById('lyrics-drawer-modal');
    if (!drawer) return;
    drawer.classList.toggle('hidden');
    if (!drawer.classList.contains('hidden')) {
      window.renderLyricsInDrawer();
    }
  };

  window.closeLyricsDrawer = function() {
    document.getElementById('lyrics-drawer-modal')?.classList.add('hidden');
  };

  window.renderLyricsInDrawer = async function() {
    const track = window.pulseState.currentTrack || { title: 'Starboy', artist: 'The Weeknd' };
    const titleEl = document.getElementById('lyrics-drawer-title');
    const artistEl = document.getElementById('lyrics-drawer-artist');
    const contentEl = document.getElementById('lyrics-drawer-content');
    
    if (titleEl) titleEl.textContent = track.title;
    if (artistEl) artistEl.textContent = track.artist;
    if (contentEl) contentEl.innerHTML = '<div style="text-align: center; padding: 2rem; color: #c084fc;"><i class="fa-solid fa-spinner fa-spin" style="font-size: 1.8rem;"></i><p style="margin-top: 0.5rem;">Syncing lyrics...</p></div>';

    // Check LRCLIB first
    let lyrics = await window.lyricsService.getLyrics(track);
    if (!lyrics || !lyrics.lines || lyrics.lines.length === 0) {
      // Fallback to Gemini AI generated lyrics
      lyrics = await window.lyricsService.generateGeminiLyrics(track.title, track.artist);
    }
    ...
    if (contentEl) {
      contentEl.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.2rem; padding-bottom: 0.8rem; border-bottom: 1px solid var(--border-glass);">
          <span style="font-size: 0.75rem; color: #c084fc; font-weight: 700; background: rgba(168,85,247,0.15); padding: 4px 10px; border-radius: 20px;">
            <i class="fa-solid fa-wand-magic-sparkles"></i> ${lyrics.source || (lyrics.isSynced ? 'Synced Karaoke' : 'Plain Lyrics')}
          </span>
          <button onclick="window.regenerateGeminiLyrics()" style="font-size: 0.78rem; font-weight: 700; color: #fff; background: var(--accent-gradient); border: none; padding: 5px 12px; border-radius: 20px; cursor: pointer; display: flex; align-items: center; gap: 0.4rem;">
            <i class="fa-solid fa-sparkles"></i> Gemini AI Lyrics
          </button>
        </div>
        <div id="lyrics-lines-container" style="display: flex; flex-direction: column; gap: 0.85rem; font-size: 1.1rem; line-height: 1.6; text-align: center;">
          ${lyrics.lines.map((l, i) => `
            <div class="lyric-line ${i === 0 ? 'active-lyric' : ''}" id="lyric-line-${i}" style="color: ${i === 0 ? '#fff' : 'rgba(255,255,255,0.45)'}; font-weight: ${i === 0 ? '800' : '500'}; transition: all 0.25s ease; font-size: ${i === 0 ? '1.25rem' : '1.05rem'}; cursor: pointer;"
              onclick="if(${l.time !== null}) { window.PulsePlaybar.handleSeekEnd((${l.time} / (window.PulsePlaybar.getState().duration || 200)) * 100); }">
              ${l.text}
            </div>
          `).join('')}
        </div>
  ...
  window.regenerateGeminiLyrics = async function() {
    const track = window.pulseState.currentTrack || { title: 'Starboy', artist: 'The Weeknd' };
    const contentEl = document.getElementById('lyrics-drawer-content');
    if (contentEl) {
      contentEl.innerHTML = '<div style="text-align: center; padding: 2rem; color: #c084fc;"><i class="fa-solid fa-wand-magic-sparkles fa-bounce" style="font-size: 2rem;"></i><p style="margin-top: 0.5rem;">Gemini 3.6 Flash is composing full lyrics & translations...</p></div>';
    }
    const aiLyrics = await window.lyricsService.generateGeminiLyrics(track.title, track.artist);
    window.pulseState.currentLyrics = aiLyrics;
    window.renderLyricsInDrawer();
    window.showToast('Lyrics generated by Gemini AI!', 'success', 2500);
  };
