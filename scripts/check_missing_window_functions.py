...
functions_to_check = [
    ...
    'openFullscreenPlayerWithLyrics',
    'openGenreGridView',
    'openLoginModal',
    'openLyricsForTrack',
    'openSignupModal',
    'playArtistTopTracks',
    'playCurrentArtistTracks',
    'playGenreTracks',
    'playLyricsModalTrack',
    'playSpecificTrack',
    'publishAdminTrack',
...
for fn in functions_to_check:
    pattern = rf'window\.{fn}\s*='
    if not re.search(pattern, combined):
        missing.append(fn)

print(f"Total checked: {len(functions_to_check)}")
print(f"MISSING functions on window: {len(missing)}")
for m in missing:
    print(f"  ❌ window.{m} is NOT defined!")
